...
<table  id="i-tbltipoimovel" class="table table-striped table-bordered table-hover" >
    <thead class="thead-dark">
        <tr >
            ...
            <th style="text-align:center"> Tipo de Comércio </th>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        <label class="control-label">Tipo de Comércio</label>
...
<script>
    ...
    function carga()
    {
        url = "{{route('tipocomercio.carga')}}/{{Auth::User()->IMB_IMB_ID}}";
        ...
        $.getJSON( url, function( data)
        {
            ...
            {
                var prefixo = '';
                var datainativo = moment( data[nI].IMB_TPC_DTHINATIVO ).format('DD/MM/YYYY');
                ...
                linha = 
                        '<tr>'+
                        '<td style="text-align:center valign="center">'+data[nI].IMB_TPC_ID+'</td>' +
                        '<td style="text-align:center valign="center">'+data[nI].IMB_TPC_DESCRICAO+'</td>' +
                        '<td style="text-align:center valign="center">'+datainativo+'</td>' +
                        '<td style="text-align:center" valign="center"> '+
                        '<a href=javascript:editar('+data[nI].IMB_TPC_ID+') class="btn btn-sm btn-primary"><span class="glyphicon glyphicon-pencil"></span></a> '+
                        '<a href=javascript:apagar('+data[nI].IMB_TPC_ID+') class="btn btn-sm btn-danger"><span class="glyphicon glyphicon-trash"></span></a> '+
                        '</td> '+
                        '</tr>';
    ...
    function editar( id )
    {
        ...
        url = "{{route('tipocomercio.buscar')}}/"+id;
        ...
        $.ajax(
        {
            ...
            {
                $("#i-id").val( data.IMB_TPC_ID );
                $("#i-nome-alteracao").val( data.IMB_TPC_DESCRICAO );
                $("#modalalteracao").modal('show');
    ...
    function gravar()
    {
        ...
        if ( $("#i-id").val() == '' )
            url = "{{route('tipocomercio.store')}}"
        else
            url = "{{route('tipocomercio.update')}}";
        ...
        atm =
        {
            ...
            IMB_TPC_DESCRICAO :  $("#i-nome-alteracao").val(),
        }
    ...
    function apagar( id )
    {
        ...
        }).then((result) => 
        {
            if (result.value) 
            {
                var url = "{{ route('tipocomercio.apagar')}}/"+id;
