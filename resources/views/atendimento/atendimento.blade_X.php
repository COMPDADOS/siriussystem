...
-->
          ...
          <input type="hidden" id="i-idatendimento" value="{{ $id }}">
          <div class="portlet box blue">
            <div class="portlet-title">
              <div class="caption">
                    <i id="i-atendimentoinfo" class="fa fa-gift"></i>
              </div>
                ...
                <div id="i-div-atendimento">
                  <div class="row">
                    <div class="col-md-12">
                      ...
                      <div class="col-md-2">
                          <label  id="i-encerrado" class="form-control div-center">ENCERRADO</label>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        $.post( url, telefone, function(data)
        {
          ...
          atm =
              { 
                  ...
                  VIS_ATA_HORA: moment().format('HH:MM'),          
...
function cargaSelecionados()
{
  ...
  url = "{{ route( 'cargaselecionados-efetivos' ) }}";
  ...
  $.ajax(
    ...
    //                    '           <button class="btn btn-sm btn-danger" onclick="apagarCorImo('+data[nI].IMB_CORIMO_ID+' )">Apagar</button>'+ 
                  '<a  class="btn btn-sm btn-danger" onclick="apagarImovelSelecaoEfetivo('+data[nI].VIS_ATI_ID+')">Apagar</a>'+
            '    </td>'+
            '</tr>';
...
function apagarImovelSelecaoEfetivo( id )
{
  cliente = 
  {
    VIS_ATI_ID : id
  ...
  url = "{{ route( 'apagarimvselecefetivo' ) }}";

  //console.log( url+'?VIS_ATI_ID='+id);
    ...
    $.ajax(
    {
      ...
      success: function (data) 
      {
        ...
        if( id == 0 ) 
        {
          ...
          $("#i-horaagenda").val (RetornaHoraAtual());
...
function gravarEncerramentoRegAgenda( $operacao )
{
  ...
  { 
    ...
    VIS_ATA_ID : $("#i-idrelato").val(),
	  ...
	  VIS_ATA_DATAENCERRAMENTO: $("#i-datafechamento").val(),
	  VIS_ATA_HORAENCERRAMENTO: RetornaHoraAtual(),
	  VIS_ATD_IDINICIO:$("#i-idusuario").val(),
    VIS_ATA_OBSERVACOES:$("#i-observacoes").val(),
//	  VIS_ATA_DATAHORA:
	  //VIS_ATA_DATAHORAFECHADO:
...
function gravarFechamento()
{
  ...
  else
  {
    ...
    { 
      ...
      IMB_ATM_DTHFIM: dataYYYYMMDD( RetornaDataHoraAtual())+' '+RetornaHoraAtual(),
    }
