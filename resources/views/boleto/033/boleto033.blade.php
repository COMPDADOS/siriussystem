...
<html>
  ...
  <div class="row row-bottom-margin">
       <div class="col-xs-2 div-left">
           <img src="https://www.siriussystem.com.br/sys/storage/images/{{$im->IMB_IMB_ID}}/logos/logoboleto.jpg" alt="alt-logo">
       </div>
       <div class="col-xs-6 div-center">
           ...
           <p style="margin: -2;" class="titulo-10-black" >Fones:{{$im->IMB_IMB_TELEFONE1 }}- Creci: {{$im->IMB_IMB_CRECI}}</p>
       </div>
       <div class="col-xs-3 div-center">
           ...
           <p style="margin: -2;"><span class="sub-titulo-nome">{{$dadosboleto["IMB_CTR_REFERENCIA"]}} </span></p>
    ...
    <div class="row row-bottom-margin">
      <div class="col-xs-12 sub-titulo-nome-left">
                   Imóvel: <span class="titulo-11-black-italic">{{$dadosboleto["IMB_IMV_OBJETOLOCACAO"]}}</span>
       </div>
      
    </div>
    <div class="row-top-margin">
      <div class="col-xs-2 titulo-10-black-left">
          Término Contrato {{date('d/m/Y',strtotime( $dadosboleto["IMB_CTR_TERMINO"]))}} 
      </div> 
      <div class="col-xs-2 titulo-10-black-left">
          Próximo Reajuste {{date('d/m/Y',strtotime($dadosboleto["IMB_CTR_DATAREAJUSTE"]))}} 
      </div> 
      <div class="col-xs-2 titulo-10-black-left">
          Nosso Número {{$dadosboleto["nosso_numero"]}}
      </div> 
      <div class="col-xs-2 titulo-10-black-left">
          Seu Número {{str_pad($dadosboleto["numero_documento"], 10, 0, STR_PAD_LEFT)  }} 
      </div> 
      <div class="col-xs-2 titulo-11-black">
        @php
           $valortotal = $dadosboleto["valor_boleto"];
        @endphp
        Valor Documento<br>
        R$ {{$valortotal}}
    ...
    <div class="div-demonstrativo ">
      ...
      <table  id="tbleventos" class="table row-top-margin table-condensed table-hover table-bordered table-striped">
        ...
        <tbody>
            ...
            @foreach( $cpi as $item)
              <tr>
                <td class="demonstrativo">{{$item->IMB_TBE_ID}}</td>
                <td class="demonstrativo">{{$item->IMB_TBE_DESCRICAO}}</td>
                <td class="demonstrativo">{{$item->IMB_RLT_LOCATARIOCREDEB}}</td>
                <td class="demonstrativo div-right">{{ number_format($item->IMB_LCF_VALOR, 2, ',', '.')}}</td>
                <td class="demonstrativo">{{$item->IMB_LCF_OBSERVACAO}}</td>
              </tr>
            @endforeach
        </tbody>
      </table>                  
    ...
    <TR>
      ...
      <TR>
        <TD colspan=4 class=BoletoLogo><img src="{{asset('/global/img/logobanco033.png')}}"></TD>
        <TD colspan=2 class=BoletoCodigoBanco>033/TD>
