...
td.BoletoCodigoBanco {font-size: 6mm; font-family: arial, verdana; font-weight : bold;
  ...
  <TABLE class=Boleto id="TABLE1" onclick="return TABLE1_onclick()">
    <TR>
      ...
      <TR>
        <TD colspan=3 class=BoletoLogo-sup><img src="{{env('APP_URL')}}/storage/images/{{$im->IMB_IMB_ID}}/logos/logoboleto.png" alt="alt-logo"></TD>
        ...
        <TD colspan=2 class=demonstrativo-center>Vencimento <p>{{$dadosboleto["data_vencimento"]}}</p>
              <p>Valor</p>
      ...
      <TR>
      <TD colspan=6 class=demonstrativo-center><b><u>Imóvel: <p>{{$dadosboleto["IMB_IMV_OBJETOLOCACAO"]}}</p></u></b></TD>
      <TD colspan=2 class=demonstrativo-center><b><u>Próx. Reajuste: <p>{{$dadosboleto["IMB_CTR_DATAREAJUSTE"]}} </p></u></b></TD>
      <TD colspan=2 class=demonstrativo-center><b><u>Término Contrato: <p>{{$dadosboleto["IMB_CTR_TERMINO"]}}</p></u></b></TD>
      </TR>
    </TR>
  </TABLE>
  ...
  <div class="div-demonstrativo ">
    ...
    <TR>
      ...
      <TR>
        ...
        <TD colspan=4  class=itens-left><b><u>Obervacao</u></b></TD>
      </TR>

        @foreach( $cpi as $item)
          <TR>
            <TD colspan=4 class=itens-left>{{app( 'App\Http\Controllers\ctrRotinas')->pegaNomeEvento($item->IMB_TBE_ID)}}</TD>
            <TD colspan=2  class=itens-left>{{$item->IMB_RLT_LOCATARIOCREDEB}}</TD>
            <TD colspan=2  class=itens>R$ {{ number_format($item->IMB_LCF_VALOR, 2, ',', '.')}}</TD>
            <TD colspan=4  class="itens-left" >{{$item->IMB_LCF_OBSERVACAO}}</TD>
          </TR>
        @endforeach
      </TABLE>
  </div>
    ...
    <TR>
      ...
      <TR>
        <TD colspan=8 Class=BoletoValorSacado>Imóvel:({{$dadosboleto["IMB_IMV_ID"]}}){{$dadosboleto["IMB_IMV_OBJETOLOCACAO"]}}</TD>
        <TD colspan=2 Class="BoletoValorSacado div-center" >Vencimento</TD>
        <TD colspan=2 Class="BoletoValorSacado div-center" >Valor</TD>
      </TR>      
      <TR>
        ...
        <TD colspan=2 Class="BoletoValorSacado div-center">{{$dadosboleto["valor_boleto_impresso"]}}</TD>
