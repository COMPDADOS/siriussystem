...
td.BoletoCodigoBanco {font-size: 6mm; font-family: arial, verdana; font-weight : bold;
  ...
  <div class="container">
    <button class="btn btn primary botao"  onClick="impressao()">Impressão</button>
    <button class="btn btn danger botao"   onClick="window.close()">Sair</button>
    </div>

  @foreach( $dbl as $dadosboleto )
  @php
        $im = app('App\Http\Controllers\ctrImobiliaria')->pegarImobiliaria( $dadosboleto->IMB_IMB_ID );
      @endphp
  ...
  <TABLE class=Boleto id="TABLE1" onclick="return TABLE1_onclick()">
    <TR>
      ...
      <TR>
        <TD colspan=3 class=BoletoLogo-sup><img src="{{env('APP_URL')}}/storage/images/{{$im->IMB_IMB_ID}}/logos/logoboleto.jpg" alt="alt-logo"></TD>
        <TD colspan=7 class=BoletoLogo-sup>           
        ...
        <TD colspan=1 class=demonstrativo-center>Vencimento <p>{{$dadosboleto->data_vencimento}}</p>
              <p>Valor</p>
              <p> <b> R$ {{$dadosboleto->valor_boleto_impresso}}</b></p>
            </TD>
        </TD>
      </TR>
      <TR>
        <TD colspan=11 class=Boletorisco>&nbsp;</TD>
      </TR>
      <TR>
        <TD colspan=11 class=demonstrativo-center><b><u>Demonstrativo</u></b></TD>
      </TR>
    </TR>
  </TABLE>
  ...
  <div class="div-demonstrativo ">
    ...
    <TR>
      ...
      <TR>
        ...
        <TD colspan=1  class=itens-left></TD>
      ...
      @php
              $cpi = app('App\Http\Controllers\ctrCobrancaGerada')->cargaItensPermanenteSemJson( $dadosboleto->IMB_CGR_ID);
            @endphp
      @foreach( $cpi as $item)
          <TR>
            <TD colspan=4 class=itens-left>{{$item->IMB_TBE_DESCRICAO}}</TD>
            <TD colspan=1  class=itens-left>{{$item->IMB_RLT_LOCATARIOCREDEB}}</TD>
            <TD colspan=2  class=itens>R$ {{ number_format($item->IMB_LCF_VALOR, 2, ',', '.')}}</TD>
    ...
    <TR>
      ...
      <TR>
        <TD colspan=4 class=BoletoLogo><img src="{{asset('/global/img/logobanco084.png')}}"></TD>
        <TD colspan=2 class=BoletoCodigoBanco>084-1</TD>
      ...
      @php
        $endereco = app('App\Http\Controllers\ctrRotinas')->pegarEnderecoCobranca( $dadosboleto[ "IMB_CTR_ID"]);
      @endphp
      <TR>
        <TD rowspan=3 Class=BoletoTituloSacado>Sacado:</TD>
        <TD colspan=8 Class=BoletoDadosSacado> {{$endereco->IMB_CCB_DESTINATARIO}}</TD>
      </TR>
      <TR>
        <TD colspan=10 Class=BoletoDadosSacado>{{$endereco->IMB_CCB_ENDERECO.' '.
          $endereco->IMB_CCB_ENDERECONUMERO.' '.$endereco->IMB_CCB_ENDERECOCOMPLEMENTO}}</TD>
      <TR>
        <TD colspan=10 Class=BoletoDadosSacado>
          {{$endereco->IMB_CCB_CEP.' - '.$endereco->CEP_CID_NOME.'('.$endereco->CEP_UF_SIGLA.')'}}</TD>
      </TR>
      </TR>
      <TR>
      ...
      <TR>
        <TD colspan=11 height=60 valign=top><img src="data:image/png;base64,{{base64_encode( $dadosboleto->barcode )}}"></TD>
      </TR>
      <tr><td colspan=11 class=BoletoPontilhado>&nbsp;</td></tr>
      <TR>
