...
<body style="background-color: #ffffff">
    ...
    <table id="TABLE1" onclick="return TABLE1_onclick()">
        ...
        <tr></tr>
            ...
            <td colspan="8" align="center">
                <h1>
                    Relatório de Recebimentos de Aluguéres
                </h1>
                <h3>
        ...
        @foreach ( $rec as $recibo )
            ...
            <tr class="bg-1">
                <td   class="div-center semborda"  width="15%" style="height: 21px">
                    Locatário: <p><b>{{ $recibo->NOMELOCATARIO }}</b></p>
                </td>
                ...
                <td  class="div-center semborda"width="15%" style="height: 21px">
                    Vencimento: <p><b>{{ app('App\Http\Controllers\ctrRotinas')->formatarData( $recibo->IMB_RLT_DATACOMPETENCIA)}}</b></p>
                </td>
                <td class="div-center semborda"width="15%" style="height: 21px">
                    Pagamento: <p><b>{{ app('App\Http\Controllers\ctrRotinas')->formatarData( $recibo->IMB_RLT_DATAPAGAMENTO)}}</b></p>
                </td>
                <td  class="div-center semborda" width="15%" style="height: 21px">
                    $ Pago: <p>{{number_format(app('App\Http\Controllers\ctrRecibolocatario')->totaldoRecibo( $recibo->IMB_RLT_NUMERO ),2,",",".")}}</p>
                </td>
            </tr>
            @php
            ...
            @foreach ( $itens as $item)
                <tr class="semborda">
                    ...
                    <td class="div-right semborda" width="10%">
                        {{ number_format( $item->IMB_RLT_VALOR,2,",","." ) }}
                    </td>
                    <td class="div-center semborda" width="3%">
                        {{$item->IMB_RLT_LOCATARIOCREDEB}}
                    </td>
                    <td class="div-left semborda" width="20%">
                        {{$item->IMB_RLT_OBSERVACAO}}
                    </td>
                </tr>
            @endforeach
    ...
    <center><h1>Resumo dos Recebimentos</h1></center><br/>
    <div class="div-center font-20">
        <table>
            ...
            <tbody>
                ...
                @php
                    $resumoitem = app('App\Http\Controllers\ctrRecibolocatario')->
                    resumoRecebidoPeriodo( $datainicio, $datafim, '','' );
                @endphp
                    <tr >
                        <td class="div-right" width="10%">{{ $resumoitem[0] }}</td>
                        <td class="div-right" width="10%">{{ $resumoitem[1] }}</td>
                        <td class="div-right" width="10%">{{ $resumoitem[2] }}</td>
                        <td class="div-right" width="10%">{{ $resumoitem[3] }}</td>
                        <td class="div-right" width="10%">{{ $resumoitem[4] }}</td>
                        <td class="div-right" width="10%">{{ $resumoitem[5] }}</td>
                        <td class="div-right" width="10%">{{ $resumoitem[6] }}</td>
                        <td class="div-right" width="10%">{{ $resumoitem[7] }}</td>
                        <td class="div-right" width="20%"><b>{{ $resumoitem[8] }}</b></td>
                    </tr>
            </tbody>
        </table>
    </div>
