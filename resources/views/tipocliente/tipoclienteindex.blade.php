...
<script>
    ...
    function carga()
    {
        url = "{{route('tipocliente.carga')}}/{{Auth::User()->IMB_IMB_ID}}";
        ...
        $.getJSON( url, function( data)
        {
            ...
            {
                var prefixo = '';
                var datainativo = moment( data[nI].IMB_TIPCLI_DTHINATIVO ).format('DD/MM/YYYY');
                ...
                linha = 
                        '<tr>'+
                        '<td style="text-align:center valign="center">'+data[nI].IMB_TIPCLI_ID+'</td>' +
                        '<td style="text-align:center valign="center">'+data[nI].IMB_TIPCLI_DESCRICAO+'</td>' +
                        '<td style="text-align:center valign="center">'+datainativo+'</td>' +
                        '<td style="text-align:center" valign="center"> '+
                        '<a href=javascript:editar('+data[nI].IMB_TIPCLI_ID+') class="btn btn-sm btn-primary"><span class="glyphicon glyphicon-pencil"></span></a> '+
                        '<a href=javascript:apagar('+data[nI].IMB_TIPCLI_ID+') class="btn btn-sm btn-danger"><span class="glyphicon glyphicon-trash"></span></a> '+
                        '</td> '+
                        '</tr>';
    ...
    function editar( id )
    {
        ...
        url = "{{route('tipocliente.buscar')}}/"+id;
        ...
        $.ajax(
        {
            ...
            {
                $("#i-id").val( data.IMB_TIPCLI_ID );
                $("#i-nome-alteracao").val( data.IMB_TIPCLI_DESCRICAO );
                $("#modalalteracao").modal('show');
    ...
    function gravar()
    {
        ...
        if ( $("#i-id").val() == '' )
            url = "{{route('tipocliente.store')}}"
        else
            url = "{{route('tipocliente.update')}}";
        ...
        atm =
        {
            ...
            IMB_TIPCLI_DESCRICAO :  $("#i-nome-alteracao").val(),
        }
    ...
    function apagar( id )
    {
        ...
        }).then((result) => 
        {
            if (result.value) 
            {
                var url = "{{ route('tipocliente.apagar')}}/"+id;
