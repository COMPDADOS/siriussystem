        ...
        <div class="row">
            <div class="col-md-12">
                <table class="table table-striped table-hover"  id="resultTable">
                    <thead>
                        <th width="30%">Nome&nbsp;<i title='Locador' class='fas fa-child' style='color:blue; font-weight: bold;'></i>=Locador - 
                                            &nbsp;<i title='Locatário' class='fas fa-child' style='color:orange'></i>=Locatário - 
                                            &nbsp;<i title='Interessado' class='fas fa-child ' style='color:peru;'></i>=Interessado - 
                                            &nbsp;<i title='Fiador' class='fas fa-child' style='color:black'></i>=Fiador
                </th>
                        <th width="30%">Telefone</th>
                        <th width="10%">Corretor</th>
                        <th width="5%"></th>
                        <th width="5%"></th>
                        <th width="5%">Atualizaçao</th>
    ...
    "oPaginate": {
        ...
        "sLast": "Último"
           ...
           bLengthChange: false,
            ...
            columns: [
                {data: 'IMB_CLT_NOME', render:getNome },
            ...
            "columnDefs": [
                ...
                } ,
                {
                    ...
                    "defaultContent": "<div style='text-align:center'>"+
                        "<button class='btn green-meadow glyphicon glyphicon-search pull-right btn-sm show-imv'></button>"+
                        "@php  $acesso = app( 'App\Http\Controllers\ctrRotinas')->verificarRecurso( 'Clientes', '   ', 'CRM', 'Clientes','S', 'A', 'Botão')@endphp"+
                        "<button class='btn btn-primary btn-sm glyphicon glyphicon-pencil pull-right alt-clt {{$acesso}}'></button>"+
                        "<button title='Rastrear Atendimento' class='btn btn-warning pull-right btn-sm btn-rastrear {{$acesso}}'><i class='fa fa-bars' aria-hidden='true'></i></button>",
                },
                { width: 200, targets: 0 },
           ],
            searching: false
...
//            window.location = "{{ route('atendimento.atendimento') }}/" + id;
        ...
        function getFonesCliente( data )
        {
            ...
            '<div class=" font-10 {{$acesso}}"><i class="fa fa-phone" aria-hidden="true"></i> '+data+'</div>';
            return texto;


        }

        function getNome( data, type, full, meta )
        {

            console.log('locador '+full.LOCADOR);
            var texto = '';
            if( full.LOCADOR )
            texto = texto + "&nbsp;<i title='Locador' class='fas fa-child' style='color:blue; font-weight: bold;'></i>";

            if ( full.INTERESSADO != null)                
                texto = texto + "&nbsp;<i title='Interessado' class='fas fa-child ' style='color:peru;'></i>";
            
            if ( full.LOCATARIO != null)                
                texto = texto +  "&nbsp;<i title='Locatário' class='fas fa-child' style='color:orange'></i>";
            
            if ( full.FIADOR != null)                
                texto = texto +  "&nbsp;<i title='Fiador' class='fas fa-child' style='color:black'></i>";
            
            return "<div> "+data+texto;
        }
