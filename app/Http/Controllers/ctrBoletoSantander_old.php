...
class ctrBoletoSantander extends Controller
{
    ...
    public function index( $id, $poremail )
    {
        ...
        if( $cp ) 
        {
            ...
            $dadosboleto["agencia"] = "680"; // Num da agencia, sem digito
            $dadosboleto["conta"] = "1300627";	// Num da conta, sem digito
            $dadosboleto["conta_dv"] = "1"; 	// Digito do Num da conta
            $dadosboleto["codigo_cliente"] = "7682840"; 	// Digito do Num da conta

            // DADOS PERSONALIZADOS - ITA�
            $dadosboleto["carteira"] = "104";  // C�digo da Carteira: pode ser 175, 174, 104, 109, 178, ou 157
            ...
            $fator_vencimento = $this->fator_vencimento( date('d/m/Y',$dadosboleto["data_vencimento"]) );
            ...
            if( $poremail == 'S' )
            {
                ...
                foreach( $array as $a )
                {
                    Mail::send( 'boleto.santander.boletosantanderlink', compact( 'dadosboleto', 'im','ctr', 'imv','barcode', 'cpi','cp' ), 
            ...
            {
                $pdf=PDF::loadView('boleto.santander.boletosantander', compact( 'dadosboleto', 'im','ctr', 'imv','barcode', 'cpi' ) );
    ...
    public function modulo_11($num, $base=9, $r=0)  
    {
        ...
        for ($i = strlen($num); $i > 0; $i--) 
        {
            Log::info( 'i: '.$i.' num: '.$num.' fator: '.$fator);
            // pega cada numero isoladamente
            $numeros[$i] = substr($num,$i-1,1);
    ...
    function abastecerPermanente( $id )
    {

        $cg = mdlCobrancaGerada::find( $id );
        ...
        $dven = implode("/",array_reverse(explode("-",$cg->IMB_CGR_DATAVENCIMENTO)));
        //dd( $dven );

        $codigobanco = $conta->FIN_CCI_BANCONUMERO;
        $nummoeda = "9";
        $fixo     = "9";   // Numero fixo para a posi��o 05-05
        $ios	  = "0";   // IOS - somente para Seguradoras (Se 7% informar 7, limitado 9%)
        ...
        $cgp->IMB_CGR_SELECIONADA = $cg->IMB_CGR_SELECIONADA;
